...
#source("~/Documents/Research/Duration CIs 3 non-uniform/6 deltaCI abm summer 2021 (Dohyun, Satyaa, Ella, Hellman)/abm38.2.R")
...
simnumtaxa <- function(mintaxa, maxtaxa)  {
  # Simulate number of taxa found
  # mintaxa: min number of taxa
  # maxtaxa: max number of taxa
  # mintaxa and maxtaxa are assumed to be positive whole numbers
  if(mintaxa == maxtaxa)  { 
    numtaxa <- mintaxa
  }  else  {
    numtaxa <- sample(seq(mintaxa,maxtaxa,1), 1)
  }
  return(numtaxa)
}
...
analyzeResults <- function(ntaxa, truedelta, CIlo, CIhi, meanlambda, conf, filename)  {
  ...
  # scatterplot of lower endpoint coverage 
  plot(truedelta, CIlo)
  abline(0,1, col="red")
  title(main=paste("CI lower", round(sum(CIlo>truedelta)/nreps,3), "error"))
  
  # scatterplot of upper endpoint coverage 
  plot(truedelta, CIhi)
  abline(0,1, col="red")
...
minfinds <- 10
maxfinds <- 10
#codeversion <- "~/Documents/Research/Duration CIs 3 non-uniform/3 deltaCI abm summer 2020 (Danielle, Peem)/duration38.1.1.R"
#codeversion <- "~/Documents/Research/Duration CIs 3 non-uniform/5 deltaCI abm spring 2021 (Emma, Jiung)/duration40.3.R"
#codeversion <- "~/Documents/Research/Duration CIs 3 non-uniform/6 deltaCI abm summer 2021 (Dohyun, Satyaa, Ella, Hellman)/code version 41/duration41.4 for.R"
codeversion <- "/Users/Dohyun/Desktop/steve/duration.sims.plots.R"
