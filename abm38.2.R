...
abm38 <- function(x, distance=TRUE, ext=TRUE, base=NULL, conf=.9, PLOT=0)    {
  ...
  if(PLOT)  {
    ...
    hist(xraw, nclass=6, prob=TRUE, xlim=c(xlo,xhi), col="gray", xlab=xlabel, border="white", 
    ...
    if((distance & ext) | (!distance & !ext))
      curve(drefbeta((x-xbase)/abs(xhi-xlo), Lhat)/abs(xhi-xlo), from=xlo, to=xhi, add=TRUE)
    if((!distance & ext) | (distance & !ext))
      curve(drefbeta((x-xbase)/abs(xhi-xlo), -Lhat)/abs(xhi-xlo), from=xlo, to=xhi, add=TRUE)
